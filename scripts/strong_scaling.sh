...
for THREADS in "${THREAD_ARRAY[@]}"; do
    echo "Running $PROGRAM with problem_size=$PROBLEM_SIZE, blocksize=$BLOCKSIZE, on_demand=$ON_DEMAND, threads=$THREADS for $N_TRIES times"
    
    for ((i = 1; i <= N_TRIES; i++)); do
        echo "Iteration $i with $THREADS threads"
        $PROGRAM $PROBLEM_SIZE $BLOCKSIZE $ON_DEMAND $THREADS
    done
done
